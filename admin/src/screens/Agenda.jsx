...
function Agenda() {
  ...
  const handleAddAgenda = async (e) => {
    ...
    try {
      await axios.post('https://admin.precision-omics.org/api/addAgenda', formData);
      setDate('');
      setPdfFile(null);
      fetchAgendas();
  ...
  const handleDeleteAgenda = async (id) => {
    try {
      await axios.delete(`https://admin.precision-omics.org/api/deleteAgenda/${id}`);
      fetchAgendas();
        ...
        <ul>
          {agendas.map((agenda) => (
            <li key={agenda._id} className="agenda-item">
              ...
              <a href={`https://admin.precision-omics.org${agenda.pdf}`} target="_blank" rel="noopener noreferrer">
                View PDF
              </a>
