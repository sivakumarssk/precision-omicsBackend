...
function ManageDates() {
  ...
  const handleAddDates = async () => {
    ...
    try {
      const response = await axios.post('https://admin.precision-omics.org/api/addDates', { key, items });
  ...
  const handleAddSingleItem = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://admin.precision-omics.org/api/addSingleDate', { key, item });
      setDates(response.data.data.dates);
      setHead('');
      setDate('');
  ...
  const handleDeleteItem = async (key, id) => {
    try {
      await axios.delete(`https://admin.precision-omics.org/api/deleteDates/${id}`);
      fetchDates();
  ...
  const handleDeleteKey = async (key) => {
    try {
      await axios.delete(`https://admin.precision-omics.org/api/deleteKey/${key}`);
      fetchDates();
