...
function Schedule() {
    ...
    const handleDeleteEvent = async (day, id) => {
        try {
            const response = await axios.delete(`https://admin.precision-omics.org/api/schedule/${day}/${id}`);
    ...
    const handleSubmit = async (day) => {
        try {
            ...
            const response = await axios.patch("https://admin.precision-omics.org/api/addschedule", {
                day,
                dayDate: scheduleForm[day].dayDate,
                list: scheduleForm[day].list
            });
