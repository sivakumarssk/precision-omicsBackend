...
const SpeakerList = () => {
    ...
    const deleteItem = async (id, category) => {
        ...
        switch (category) {
            case 'speaker':
                endpoint = `https://admin.precision-omics.org/api/deleteSpeaker/${id}`;
                break;
            case 'keynote':
                endpoint = `https://admin.precision-omics.org/api/deleteKeynoteSpeaker/${id}`;
                break;
            case 'committee':
                endpoint = `https://admin.precision-omics.org/api/deleteCommitte/${id}`;
                break;
            default:
                break;
        }
                ...
                <tbody>
                    {items.map((item) => (
                        <tr key={item._id}>
                            <td>
                                <img
                                    src={`https://admin.precision-omics.org${item.image}`}
                                    alt={item.name}
